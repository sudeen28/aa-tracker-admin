...
const AIRPORTS = [
  { code: "LOS", city: "Lagos", lat: 6.5774, lng: 3.3212 },
  { code: "LHR", city: "London Heathrow", lat: 51.470, lng: -0.4543 },
  { code: "JFK", city: "New York JFK", lat: 40.6413, lng: -73.7781 },
  { code: "DXB", city: "Dubai", lat: 25.2532, lng: 55.3657 },
  { code: "CDG", city: "Paris Charles de Gaulle", lat: 49.0097, lng: 2.5479 },
  { code: "LAX", city: "Los Angeles", lat: 33.9425, lng: -118.4081 },
  { code: "ORD", city: "Chicago O'Hare", lat: 41.9742, lng: -87.9073 },
  { code: "MIA", city: "Miami", lat: 25.7959, lng: -80.2870 },
  { code: "AMS", city: "Amsterdam Schiphol", lat: 52.3105, lng: 4.7683 },
...
export default function BookingForm() {
  ...
  useEffect(() => {
    ...
    getBooking(id).then(({ booking: b }) => {
      ...
      if (b.alerts?.length) setAlerts(b.alerts.map(a => ({ type: a.type, icon: a.icon, message: a.message })));
      setBookingStatus(b.status);
      setTourCode(b.tourCode || "");
      if (b.baggageStages?.length) setBaggageStages(b.baggageStages.map(s => ({ label: s.label, icon: s.icon, description: s.description, time: s.time || "", isCurrent: s.isCurrent })));
      ...
      if (b.visaEntries?.length) setVisaEntries(b.visaEntries.map(v => ({ country: v.country, code: v.code, flag: v.flag, purpose: v.purpose, status: v.status, statusLabel: v.statusLabel, statusColor: v.statusColor, summary: v.summary, tip: v.tip, tipType: v.tipType, requirements: v.requirements || [], exemptions: v.exemptions || [], checklist: v.checklist || [] })));
      ...
      if (b.mealOptions?.length) setMealOptions(b.mealOptions.map(m => ({ icon: m.icon, label: m.label, desc: m.desc, tag: m.tag || "" })));
      if (b.assistanceOptions?.length) setAssistanceOptions(b.assistanceOptions.map(a => ({ icon: a.icon, label: a.label, desc: a.desc, category: a.category, code: a.code })));
    }).catch(e => setError(e.message)).finally(() => setFetching(false));
  }, [id]);

  const handleAirportSelect = (segIdx, direction, code) => {
    const apt = AIRPORTS.find(a => a.code === code);
    if (!apt) return;
    setSegments(segs => segs.map((s, i) => i !== segIdx ? s : {
      ...s,
      [direction + "Code"]: apt.code,
      [direction + "City"]: apt.city,
      [direction + "Lat"]: apt.lat,
      [direction + "Lng"]: apt.lng,
    }));
  };
    ...
    <div style={F}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={section}>
          ...
          <div style={grid3}>
            ...
            <div>
              ...
              <input required style={input} value={passenger.firstName} onChange={e => setPassenger(p => ({ ...p, firstName: e.target.value }))} placeholder="James" />
            </div>
            <div>
              <label style={label}>LAST NAME</label>
              <input required style={input} value={passenger.lastName} onChange={e => setPassenger(p => ({ ...p, lastName: e.target.value }))} placeholder="Mitchell" />
            </div>
            <div>
              <label style={label}>FREQUENT FLYER #</label>
              <input style={input} value={passenger.frequentFlyer} onChange={e => setPassenger(p => ({ ...p, frequentFlyer: e.target.value }))} placeholder="AA-9284710" />
            </div>
            <div>
              <label style={label}>PASSPORT</label>
              <input style={input} value={passenger.passport} onChange={e => setPassenger(p => ({ ...p, passport: e.target.value }))} placeholder="***4821" />
            </div>
          </div>
        </div>
            ...
            <div key={i} style={{ border: "1px solid #e2e8f4", borderRadius: 12, padding: "18px", marginBottom: 16, background: "#fafcff" }}>
              ...
              <div style={{ ...grid2, marginBottom: 12 }}>
                <div>
                  ...
                  <select style={input} value={seg.fromCode} onChange={e => handleAirportSelect(i, "from", e.target.value)}>
                ...
                <div>
                  ...
                  <select style={input} value={seg.toCode} onChange={e => handleAirportSelect(i, "to", e.target.value)}>
                    <option value="">Select airport...</option>
                    {AIRPORTS.map(a => <option key={a.code} value={a.code}>{a.code} — {a.city}</option>)}
                  </select>
        ...
        <div style={section}>
          ...
          <div style={grid3}>
            <div><label style={label}>CHANGES BEFORE DEP.</label><input style={input} value={fare.changesBefore} onChange={e => setFare(f => ({ ...f, changesBefore: e.target.value }))} placeholder="USD 150.00" /></div>
            <div><label style={label}>CHANGES AFTER DEP.</label><input style={input} value={fare.changesAfter} onChange={e => setFare(f => ({ ...f, changesAfter: e.target.value }))} placeholder="USD 200.00" /></div>
            <div><label style={label}>CANCEL BEFORE DEP.</label><input style={input} value={fare.cancelBefore} onChange={e => setFare(f => ({ ...f, cancelBefore: e.target.value }))} placeholder="USD 250.00" /></div>
            <div><label style={label}>CANCEL AFTER DEP.</label><input style={input} value={fare.cancelAfter} onChange={e => setFare(f => ({ ...f, cancelAfter: e.target.value }))} placeholder="Non-refundable" /></div>
        ...
        <div style={section}>
          ...
          {baggageStages.map((stage, i) => (
            <div key={i} style={{ border: "1px solid #e2e8f4", borderRadius: 10, padding: "14px", marginBottom: 10, background: stage.isCurrent ? "#f0fdf4" : "#fafcff" }}>
              <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 140px 80px", gap: 8, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  <button type="button" onClick={() => setBaggageStages(s => s.filter((_,j)=>j!==i))} style={{ padding: "4px 8px", background: "#fef2f2", color: "#dc2626", border: "none", borderRadius: 5, fontSize: 11, cursor: "pointer" }}>✕</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={section}>
          <div style={{ ...sectionTitle, justifyContent: "space-between" }}>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#64748b", cursor: "pointer" }}>
              <input type="checkbox" checked={hasLayover} onChange={e => setHasLayover(e.target.checked)} /> Include layover
            </label>
          </div>
          {hasLayover && (
            <>
              ...
              <div>
                <div style={{ ...sectionTitle, fontSize: 12, justifyContent: "space-between" }}>
                  <span>Layover Tips</span>
          ...
          {visaEntries.map((v, i) => (
            <div key={i} style={{ border: "1px solid #e2e8f4", borderRadius: 12, padding: 16, marginBottom: 12, background: "#fafcff" }}>
              ...
              <div style={{ ...grid3, marginBottom: 10 }}>
                <div><label style={label}>FLAG</label><input style={input} value={v.flag} onChange={e => setVisaEntries(vs=>vs.map((x,j)=>j===i?{...x,flag:e.target.value}:x))} placeholder="🇬🇧" /></div>
              ...
              <div style={{ ...grid2, marginBottom: 10 }}>
                <div>
                  <label style={label}>TIP TEXT</label>
                  <input style={input} value={v.tip} onChange={e => setVisaEntries(vs=>vs.map((x,j)=>j===i?{...x,tip:e.target.value}:x))} placeholder="Key tip for this country" />
                </div>
                <div>
...
").filter(Boolean)}:x))}
        ...
        <div style={section}>
          ...
          {hasSeatConfig && (
            <div style={{ ...grid2 }}>
              ...
              <div><label style={label}>FLIGHT LABEL</label><input style={input} value={seatConfig.flightLabel} onChange={e => setSeatConfig(s=>({...s,flightLabel:e.target.value}))} placeholder="AA 0081 · LOS → LHR" /></div>
        ...
        <div style={section}>
          ...
          {mealOptions.map((m, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "50px 1fr 2fr 80px 60px", gap: 8, marginBottom: 8 }}>
        ...
        <div style={section}>
          ...
          {assistanceOptions.map((a, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "50px 1fr 2fr 120px 80px 60px", gap: 8, marginBottom: 8 }}>
...
// NOTE: The above file is being replaced entirely below.
// This append is intentional — see the python rewrite below.
